...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	/**
	 * 是否允许用重新注册一个相同的名字，不同的含义的definition
	 */
	...
	/**
	 * Map of singleton and non-singleton bean names, keyed by dependency  .
	 */
	...
	public void preInstantiateSingletons() throws BeansException {
		...
		for (String beanName : beanNames) {
			RootBeanDefinition bd = getMergedLocalBeanDefinition(beanName);
			// 不是抽象，是单例，不是懒加载的 RootBeanDefinition
			if (!bd.isAbstract() && bd.isSingleton() && !bd.isLazyInit()) {
				// 对实现 FactoryBean接口的类进行实例化
				if (isFactoryBean(beanName)) {
		...
		for (String beanName : beanNames) {
			...
			// 实现了 SmartInitializingSingleton接口的类处理
	...
	/**
	 *
	 * 通过descriptor，当前BeanName，需要注入的Bean全限定，和类型转换器
	 * @param descriptor         当前字段的描述
	 * @param beanName           当前bean的名称
	 * @param autowiredBeanNames 需要注入字段的beanName 列表
	...
	public Object doResolveDependency(DependencyDescriptor descriptor, @Nullable String beanName,
		...
		try {
			...
			if (matchingBeans.size() > 1) {
				// 根据descriptor在matchingBeans中找到对应的bean的全限定名
				autowiredBeanName = determineAutowireCandidate(matchingBeans, descriptor);
				if (autowiredBeanName == null) {
					// 没有找到，看看是不是有设置默认值，有的话直接发返回默认值
			...
			if (instanceCandidate instanceof Class) {
				/** 获取要被注入的实例，通过BeanFactory.getBean()获取**/
